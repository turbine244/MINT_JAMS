...
public class YouTubeService {
    ...
    // public VideoDataDTO getVideoData(String videoId, String apiKey, String
    // flaskUrl) throws Exception {
    // // 동영상 세부 정보 URL
    // String videoInfoUrl =
    ...
    // HttpResponse videoResponse = client.execute(videoRequest);
    // BufferedReader videoReader = new BufferedReader(new
    // InputStreamReader(videoResponse.getEntity().getContent(),
    ...
    //
    // // JSON 파싱
    // JsonElement videoJsonElement =
    // JsonParser.parseString(videoJsonResponse.toString());
    ...
    // videoItems.get(0).getAsJsonObject().getAsJsonObject("snippet");
    // JsonObject videoStatistics =
    // videoItems.get(0).getAsJsonObject().getAsJsonObject("statistics");
    //
    ...
    ////
    //// do {
    //// HttpGet commentRequest = new HttpGet(commentUrl + (nextPageToken != null ?
    // "&pageToken=" + nextPageToken : ""));
    //// HttpResponse commentResponse = client.execute(commentRequest);
    //// BufferedReader commentReader = new BufferedReader(new
    // InputStreamReader(commentResponse.getEntity().getContent(), "UTF-8"));
    //// StringBuilder commentJsonResponse = new StringBuilder();
    //// String commentLine;
    ////
    ...
    // JsonParser.parseString(commentJsonResponse.toString());
    //// JsonObject commentJsonObject = commentJsonElement.getAsJsonObject();
    ////
    //// if (commentJsonObject.has("items") &&
    // !commentJsonObject.get("items").isJsonNull()) {
    ...
    //// nextPageToken = null;
    //// }
    //// } while (nextPageToken != null && totalComments < 100); // 댓글 수가 100개 미만일 때
    // 계속 반복
    //
    ...
    // private VideoDataDTO sendJsonToFlaskServer(VideoDataDTO videoDataDTO, String
    // flaskUrl, JsonObject outputJson) {
    //
    ...
    // StringEntity entity = new StringEntity(outputJson.toString(),
    ...
    //
    // if (statusCode != 200) {
    // String errorMessage = "Flask 서버와의 통신 중 오류 발생: " + statusCode + ". 응답 본문: " +
    // responseBody;
